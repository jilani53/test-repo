...
if ( ! class_exists( 'SPF_SMART_BRANDS_Metabox' ) ) {
	/**
	 *
	 * Metabox Class
	 *
	 ...
	 * @version 1.0.0
	 */
	class SPF_SMART_BRANDS_Metabox extends SPF_SMART_BRANDS_Abstract {
		/**
		 * Property.
		 *
		 * @var string
		...
		public function __construct( $key, $params = array() ) {
			...
			add_action( 'wp_ajax_smart_brands_preview_meta_box', array( $this, 'smart_brands_preview_meta_box' ) );
		...
		public function add_meta_box_content( $post, $callback ) {
						...
						<div class="csf-wrapper csf-show-all">
							<div class="csf-content">
								<div class="csf-sections">
									<div class="csf-field csf-field-preview">
										<div class="sp-brand-preview-box">
											<div id="sp-brand-preview-box">
											</div>
										</div>
										<div class="clear"></div>
									</div>
			...
			foreach ( $this->sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						SPF_SMART_BRANDS::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
			...
			if ( ! empty( $this->args['show_restore'] ) || ! empty( $this->args['show_reset'] ) ) {
				...
				echo '<span class="button csf-button-reset">' . esc_html__( 'Reset', 'smart-brands-for-woocommerce' ) . '</span>';
				echo '<span class="button csf-button-cancel">' . sprintf( '<small>( %s )</small> %s', esc_html__( 'update post', 'smart-brands-for-woocommerce' ), esc_html__( 'Cancel', 'smart-brands-for-woocommerce' ) ) . '</span>';
				echo '</label>';
				echo '</div>';
			...
			if ( $is_preview ) {
				echo '<a class="btn btn-success" id="sp-brand-show-preview" data-id="' . esc_attr( $post->ID ) . '"href=""> <i class="fa fa-eye" aria-hidden="true"></i> Show Preview</a>';
			}
		...
		public function smart_brands_preview_meta_box() {
			...
			$views_id = absint( $request['post_ID'] );
			$request  = $request['sp_smart_brand_metaboxes'];
			if ( ! empty( $request ) ) {
				foreach ( $this->sections as $section ) {
			...
			Manager::views_html( $views_id, $views_meta, $title );
			die();
		}
	}
}
